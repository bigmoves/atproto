...
import { DescriptionCard } from '../atoms/description-card.tsx'
import { ButterflyIcon } from '../atoms/icons.tsx'
import { LangProp } from '../atoms/lang-string.tsx'
...
/**
 * v2 fork of `#/components/utils/scope-description.tsx`. Same AT Protocol
 * scope-parsing logic (kept in sync deliberately, see the redesign's
 * "don't import from v1" boundary) — only the presentational
 * `DescriptionCard` primitive it renders into looks different.
 */
...
function ItemDescription({
  ...
  return (
    <em {...attrs} className={`text-ink-light ${className}`}>
      {children}
    </em>
  )
}
...
function Identifier({
  ...
  return (
    <code {...attrs} className={`text-ink-light ${className}`}>
      {children}
    </code>
  )
}
